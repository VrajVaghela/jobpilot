...
│   ├── auth/
│   │   └── callback/
│   │       └── route.ts                   → OAuth callback — exchanges insforge_code, sets session cookies
│   ├── dashboard/
│   │   ├── layout.tsx                     → Auth guard (getCurrentUser → redirect /login)
...
│   └── api/
│       ├── auth/
│       │   └── refresh/route.ts           → Session refresh (createRefreshAuthRouter)
│       ├── agent/
...
├── actions/
│   ├── auth.ts                            → OAuth init (signInWithProvider) + signOut Server Actions
│   ├── profile.ts                         → Profile save + update
│   └── jobs.ts                            → Job status updates
├── proxy.ts                               → Next.js 16 Proxy — refresh session + protect routes
├── components/
│   ├── ui/                                → shadcn/ui components only
│   ├── auth/
│   │   └── OAuthButtons.tsx               → Google + GitHub sign-in buttons (client)
│   ├── layout/
│   │   ├── Navbar.tsx
│   │   └── Footer.tsx
│   ├── homepage/
...
- Provider: InsForge Auth via `@insforge/sdk` (the `@insforge/sdk/ssr` helpers)
...
- Session: server-owned cookies set by the SDK helpers —
  `insforge_access_token` (readable, used as per-request bearer) and
  `insforge_refresh_token` (httpOnly). Never hand-roll token storage.
- OAuth is server-driven (browser SSR client does NOT auto-exchange):
  1. `/login` button → `signInWithProvider()` Server Action calls
     `createAuthActions().signInWithOAuth(provider, { redirectTo, skipBrowserRedirect: true })`,
     stores the returned `codeVerifier` in an httpOnly cookie, redirects to `data.url`
  2. Provider returns to `/auth/callback` Route Handler with `insforge_code`
  3. Handler calls `createAuthActions().exchangeOAuthCode(code, codeVerifier)` →
     sets session cookies → redirects to /dashboard
- Route protection (Next.js 16 → `proxy.ts`, not `middleware.ts`):
  `updateSession()` from `@insforge/sdk/ssr/middleware` refreshes cookies on every
  request; proxy redirects unauthenticated requests on protected paths to /login.
  Each protected route group ALSO verifies via `getCurrentUser()` in a server
  layout (defense in depth — catches expired/invalid tokens).
- `/api/auth/refresh` route (`createRefreshAuthRouter`) is called by the browser
  client when the access token is missing or near expiry.
- On login → redirect to /dashboard. Sign out → `signOut()` Server Action.
- Dashboard config dependency: Google + GitHub providers must be enabled and the
  callback URL added to `allowedRedirectUrls` in the InsForge dashboard.

---

## InsForge Client Pattern

The package is `@insforge/sdk` (there is no `@insforge/ssr` package — the SSR
helpers live under the `@insforge/sdk/ssr` subpath). Config (`baseUrl`/`anonKey`)
auto-resolves from `NEXT_PUBLIC_INSFORGE_URL` / `NEXT_PUBLIC_INSFORGE_ANON_KEY`.

Three usages — never mix them:

```typescript
// lib/insforge-client.ts
// Browser-side — Client Components (DB/Storage/Realtime + getCurrentUser).
// NOTE: the browser client has NO auth mutations (no signIn/signOut) — those run
// server-side through Server Actions / Route Handlers so cookies can be written.
import { createBrowserClient } from "@insforge/sdk/ssr";
export const insforge = createBrowserClient({ refreshUrl: "/api/auth/refresh" });

// lib/insforge-server.ts
// Server-side — Server Components, API routes, Server Actions, agent code.
// Reads the access-token cookie and uses it as the per-request bearer token.
...
// actions/auth.ts — auth mutations that must SET cookies use createAuthActions
import { cookies } from "next/headers";
import { createAuthActions } from "@insforge/sdk/ssr";
const auth = createAuthActions({ cookies: await cookies() });
// auth.signInWithOAuth(...), auth.exchangeOAuthCode(...), auth.signOut()
```

Get the current user in a server context with `getCurrentUser()` (NOT `getUser()`):

```typescript
const insforge = await createInsforgeServer();
const { data: { user } } = await insforge.auth.getCurrentUser();
if (!user) redirect("/login");
// user fields: { sub (user id), email, role }
```
