...
- Utility files: camelCase — `browserbase.ts`, `insforge-client.ts`
...
```typescript
// Browser context — Client Components only (DB/Storage/Realtime + getCurrentUser)
...
// Auth mutations that set cookies — Server Actions / Route Handlers only
import { createAuthActions } from "@insforge/sdk/ssr";
```

- Never use the browser client in server context
- Never use the server client in browser context
- The browser client has no auth mutations — sign in/out run server-side via `createAuthActions`
- Current user is read with `getCurrentUser()` — never `getUser()`
...
- `@insforge/sdk` — InsForge client (SSR helpers under `@insforge/sdk/ssr`; there is no `@insforge/ssr` package)
