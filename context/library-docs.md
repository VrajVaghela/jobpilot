...
The package is `@insforge/sdk`; SSR helpers are under `@insforge/sdk/ssr`. There
is NO `@insforge/ssr` package. Config auto-resolves from
`NEXT_PUBLIC_INSFORGE_URL` / `NEXT_PUBLIC_INSFORGE_ANON_KEY` — no positional args.
...
- Browser client — Client Components: DB, Storage, Realtime, and `getCurrentUser`.
  It has NO auth mutations (no signIn/signUp/signOut).
- Server client — Server Components, API routes, Server Actions, agent functions.
- Auth mutations that set cookies (OAuth init, code exchange, sign out) use
  `createAuthActions({ cookies })` in a Server Action or Route Handler.
...
```typescript
// Get current user in server context — method is getCurrentUser(), NOT getUser()
...
OAuth is server-driven — the SSR browser client does NOT auto-exchange the
callback. Flow: a Server Action calls
`createAuthActions().signInWithOAuth(provider, { redirectTo, skipBrowserRedirect: true })`,
stores the returned `codeVerifier` in an httpOnly cookie, and redirects to
`data.url`. The `/auth/callback` Route Handler reads the `insforge_code` query
param and calls `createAuthActions().exchangeOAuthCode(code, codeVerifier)`, which
sets the session cookies. See architecture.md → Authentication for the full flow.
